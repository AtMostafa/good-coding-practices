...
def get_ccs(side1df, side2df, epoch, area, n_components, use_procrustes=False):
    ...
    for sessionData1, sessionData2 in zip(AllData1, AllData2):
        data1 = np.reshape(
            sessionData1[:, :min_trials, :min_time, :], (-1, n_components)
        )
        data2 = np.reshape(
            sessionData2[:, :min_trials, :min_time, :], (-1, n_components)
        )
        if use_procrustes:
...
def get_ccs_upper_bound(
    side1df, epoch, area, n_components, calc_percentile=True, use_procrustes=False
):
...
def get_ccs_lower_bound_monkey(
    side1df,
    side2df,
    area,
    n_components,
    len_trial,
    calc_percentile=True,
    use_procrustes=False,
):
    n_iter = params.n_iter * 10

    # get data
    AllData1 = dt._get_data_array(
        side1df, epoch_L=len_trial, area=area, model=n_components
    )
    AllData1_ = dt._get_data_array(
        side2df, epoch_L=len_trial, area=area, model=n_components
    )
    ...
    for sessionData1, sessionData2 in zip(AllData1, AllData1_):
        ...
        for n in range(n_iter):
            ...
            data1 = np.reshape(
                sessionData1_sh[:, :min_trials, :min_time, :], (-1, n_components)
            )
            data2 = np.reshape(
                sessionData2_sh[:, :min_trials, :min_time, :], (-1, n_components)
            )
            if use_procrustes:
                r.append(procrustes_wrapper(data1, data2))
            else:
...
def plot_cca(
    ...
    if dataset == "monkey":
        CCsL = get_ccs_lower_bound_monkey(
            pair_side1df,
            pair_side2df,
            area,
            n_components,
    ...
    CCsU = get_ccs_upper_bound(
        side1df, epoch, area, n_components, use_procrustes=use_procrustes
    )
    ...
    utility.shaded_errorbar(
        ax, x_, CCsU, color=params.colors.UpperCC, marker="<", ls="--"
    )
    utility.shaded_errorbar(
        ax, x_, CCsL, color=params.colors.LowerCC, marker=">", ls=":"
    )

    ax.set_ylim([-0.05, 1])
    ax.set_xlim([0.6, n_components + 0.6])
    ...
    ax.text(
        x=n_components,
        y=1,
        s=f"$n={CCsL.shape[1]}$ pairs of sessions\nacross ${n_animals}$ ${animals}$",
        ha="right",
        va="top",
    ...
    ax_hist.set_ylim([-0.05, 1])
    ax_hist.hist(
        allCCs[:4, :].mean(axis=0),
        bins=bins,
        density=True,
        label=f"Across ($n={allCCs.shape[1]}$)",
        color=params.colors.MainCC,
        alpha=0.8,
    ...
    ax_hist.hist(
        ...
        bins=bins,
        density=True,
        label=f"Within ($n={CCsU.shape[1]}$)",
        color=params.colors.UpperCC,
        alpha=0.8,
        orientation="horizontal",
    )
    ax_hist.hist(
        ...
        bins=bins,
        density=True,
        label=f"Control ($n={CCsL.shape[1]}$)",
        color=params.colors.LowerCC,
        alpha=0.8,
        orientation="horizontal",
    )
...
def plot_cca_for_ex(
    ax, example_dfs, epoch, area, n_components, dataset="monkey", prep=False
):
    if dataset == "monkey":
        animals = "monkeys"
        defs = monkey_defs
    ...
    x_ = np.arange(1, n_components + 1)
    ax.plot(
        x_, allCCs, color=params.colors.MainCC, marker="o", label=f"Across ${animals}$"
    )
    ax.plot(
        x_,
        CCsU[:, 0],
...
def VAF_pc_cc_pyal2(
    ...
    if diffTooBig:
        ...
        for i in range(n_iter):
            ...
            array_new_model = PCA(n_components=n_components, svd_solver="full").fit(
                array_new
            )

            if array1Bigger:
                PCA_model = PCA(n_components=n_components, svd_solver="full").fit(
                    rates_2
                )
            else:
                PCA_model = PCA(n_components=n_components, svd_solver="full").fit(
            ...
            pca_1_target = (
                rates_1_model.transform(rates_1_target[:, idx])
                if array1Bigger
                else rates_1_model.transform(rates_1_target)
            )
            ...
            pca_2_target = (
                rates_2_model.transform(rates_2_target)
                if array1Bigger
                else rates_2_model.transform(rates_2_target[:, idx])
            )
        ...
        VAFs1, VAFs2, R = VAF_pc_cc_pyal(
            df1, field1, epoch1, target1, df2, field2, epoch2, target2, n_components
        )
